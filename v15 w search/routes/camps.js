...
router.get("/", function(req,res){
    ...
    if(req.query.searchCamp){
      console.log(req.query.searchCamp);
      const regex = new RegExp(escapeRegex(req.query.searchCamp), 'gi');
      Campground.find({"name": regex}, function(err,foundCamps){
        if(err)
            {
              req.flash("error", "Error in searching campground");
              res.redirect("back");
            }
        else if (foundCamps.length===0)
            {
                req.flash("error", "No campgrounds found");
                res.redirect("/camps");
            }
        else{
          res.render("campgrounds/camps", {campLists : foundCamps});
        } 
      })
    }

    else //if(req.query.searchCamp.length)
        {
